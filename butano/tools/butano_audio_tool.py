...
def process_audio_files(audio_file_paths, soundbank_bin_path, soundbank_header_path, build_folder_path):
    ...
    if not audio_file_paths:
        dummy_file_path = f'{build_folder_path}/_bn_dummy_audio_file.txt'
        command.append(dummy_file_path)

        with open(dummy_file_path, 'w') as dummy_file:
            dummy_file.write('')
    else:
        command.extend(iter(audio_file_paths))
    command.extend((f'-o{soundbank_bin_path}', f'-h{soundbank_header_path}'))
    ...
    try:
        ...
        raise ValueError(
            f'mmutil call failed (return code {str(e.returncode)}): {str(e.output)}'
        )
...
def write_output_file(items, include_guard, include_file, namespace, item_class, output_file_path):
    if len(items) > 0:
        with open(output_file_path, 'w') as output_file:
            output_file.write(f'#ifndef {include_guard}' + '\n')
            output_file.write(f'#define {include_guard}' + '\n')
            output_file.write('\n')
            output_file.write(f'#include "{include_file}"' + '\n')
            output_file.write('\n')
            ...
            for item in items:
                output_file.write(
                    f'    constexpr inline {item_class} {item[0]}({item[1]});'
                    + '\n'
                )
        ...
        print(f'    {item_class}s file written in {output_file_path}')
...
def write_output_info_file(items, include_guard, include_file, namespace, item_class, output_file_path):
    with open(output_file_path, 'w') as output_file:
        ...
        pair_class = f'pair<{item_class}, string_view>'

        if len(items) > 0:
            output_file.write(f'    constexpr inline {pair_class}' + ' array[] = {' + '\n')

            for item in items:
                output_file.write(
                    (
                        f'        make_pair({item_class}({item[1]}), string_view("{item[0]}")),'
                        + '\n'
                    )
                )
            ...
            output_file.write('\n')
            output_file.write(
                f'    constexpr inline span<const {pair_class}> span(array);'
                + '\n'
            )
        else:
            output_file.write(
                f'    constexpr inline span<const {pair_class}> span;' + '\n'
            )
    ...
    print(f'    {item_class}s_info file written in {output_file_path}')
...
def write_output_files(audio_file_names_no_ext, soundbank_header_path, build_folder_path):
        ...
        for soundbank_line in soundbank_file:
            ...
            if soundbank_name.startswith('MOD_'):
                if final_name not in audio_file_names_no_ext:
                    print(f'    Music item not present in files. Skipped: {final_name}')
                else:
                ...
                if final_name not in audio_file_names_no_ext:
                    print(f'    Sound item not present in files. Skipped: {final_name}')
                else:
    ...
    write_output_file(
        ...
        'bn::music_items',
        'music_item',
        f'{build_folder_path}/bn_music_items.h',
    )
    ...
    write_output_file(
        ...
        'bn::sound_items',
        'sound_item',
        f'{build_folder_path}/bn_sound_items.h',
    )

    write_output_info_file(
        music_items_list,
        'BN_MUSIC_ITEMS_INFO_H',
        'bn_music_item.h',
        'bn::music_items_info',
        'music_item',
        f'{build_folder_path}/bn_music_items_info.h',
    )

    write_output_info_file(
        sound_items_list,
        'BN_SOUND_ITEMS_INFO_H',
        'bn_sound_item.h',
        'bn::sound_items_info',
        'sound_item',
        f'{build_folder_path}/bn_sound_items_info.h',
    )
...
def process_audio(audio_folder_paths, build_folder_path):
    ...
    file_info_path = f'{build_folder_path}/_bn_audio_files_info.txt'
    ...
    soundbank_bin_path = f'{build_folder_path}/_bn_audio_soundbank.bin'
    soundbank_header_path = f'{build_folder_path}/_bn_audio_soundbank.h'
    ...
    print(f'    Processed audio size: {str(total_size)} bytes')
